...
from DCSC_layers import *
from utils import *
from metrics import *
from criterions import *

from baselines.TransE import TransE_stymix
...
class Net(object):
    def __init__(self, args, device):
        ...
        ## self.net = ClaimH_stymix(self.Claim_pars, self.c_in, self.GCN_pars,self.c_hid, self.dropout, self.num_posts).to(self.device)
        ## self.net = BIGCN_stymix(self.c_in,self.c_hid, self.GCN_pars,self.num_posts, self.dropout).to(self.device)
        # self.net = RvNN_stymix(self.c_in, self.c_hid, self.num_posts).to(self.device)

        # self.net = VAE_stymix(self.c_in,self.c_hid,num_layers=1,bidirection=False).to(self.device)
        # self.net = VAE_stymix_claim(self.c_in,self.c_hid,num_layers=1,bidirection=False).to(self.device)

        # self.net = BIGRU_stymix(self.c_in,self.c_hid,num_layers=1,bidirection=False).to(self.device)
        # self.net = BIGRU_stymix_claim(self.c_in, self.c_hid, num_layers=1, bidirection=False).to(self.device)

        self.net = TransE_stymix(self.c_in, self.c_hid, self.num_layers_trans, self.n_head_trans, self.h_hid_trans,self.dropout).to(self.device)

        # self.net = TextCNN_stymix(self.n_filters, self.c_in, self.c_hid, self.num_posts, self.dropout).to(self.device)

        # self.net = stymix(self.c_in, self.c_hid, self.num_layers_trans, self.n_head_trans, self.h_hid_trans, self.dropout).to(self.device)
        # self.net = stymix_claim(self.c_in, self.c_hid, self.num_layers_trans, self.n_head_trans, self.h_hid_trans, self.dropout).to(self.device)
        ...
        self.loss_function = loss_function(self.margin, self.domain_num)
        # self.loss_function = loss_function_MINE(self.margin, self.Temperature,self.net.mine)
    ...
    def train_epoch(self, datapath, start_epoch,domain_dict, claim_only = False):

        # nonrumor_loader, rumor_loader = seperate_dataloader(datapath, 'train', self.batch_size,
        #                                                                 self.num_worker,
        #                                                                 self.num_posts)
        ...
        val_loader = normal_dataloader_withdomains(datapath, 'val', self.batch_size, self.num_worker, self.num_posts, domain_dict)
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            train_loss, train_acc = self.train_batch(epoch,nonrumor_loader,rumor_loader, claim_only)
            ...
            with torch.no_grad():
                val_loss, Acc_all, Acc1, Prec1, Recll1, F1, Acc2, Prec2, Recll2, F2= self.evaluation(val_loader,epoch, claim_only)
            ...
            patience -= 1
            if not patience or train_loss >6.0:
                break

        # ==================================== test model with model================================================
        # with torch.no_grad():
        #     # test_loader = normal_dataloader(datapath, 'test', self.batch_size, self.num_worker,
        #     #                                             self.num_posts)
        #
        #     #Ch-9
        #     test_loader = normal_dataloader_withdomains(datapath, 'test',self.batch_size, self.num_worker, self.num_posts,domain_dict)
        #
    ...
    def train_batch(self, epoch, nonrumor_loader, rumor_loader, claim_only):
        ...
        for iter, (Nonrumors, Rumors) in enumerate(iterloader):
            ...
            cont, sty, stypred_cont, stypred_sty, pred_cont_A, pred_sty_A, x_rec = self.net(x)
            stypred_label = torch.clone(stypred_sty).detach()

            # classification loss for teacher
            total_loss_A, styloss_cont, styloss_sty, clsloss_cont, clsloss_sty, rec_loss = \
                self.loss_function([stypred_cont, stypred_sty, pred_cont_A, pred_sty_A, x_rec, cont, sty],
                                   [stypred_label, y, x, D])
            ...
            acc_sty_A = binary_accuracy(pred_sty_A, y)
            acc_cont_A = binary_accuracy(pred_cont_A, y)

            train_loss_value += total_loss_A.item()
            acc_all += (acc_cont_A  +acc_sty_A)/2

            sys.stdout.write(
                "\r[Epoch %d/%d] [Batch %d/%d] [loss_A: %f][styacc_A: %f] ][contacc_A: %f] "
                % (
                    epoch,
                    self.epochs,
                    iter,
    ...
    def evaluation(self, dataloader, epoch):
        ...
        for iter, sample in enumerate(dataloader):
            ...
            # Step A: orginal data
            cont, sty, stypred_cont, stypred_sty, pred_cont_A, pred_sty_A, x_rec = self.net(x,original=False)
            stypred_label = torch.clone(stypred_sty).detach()

            # classification loss for original data
            total_loss_A, styloss_cont, styloss_sty, clsloss_cont, clsloss_sty, rec_loss = \
                self.loss_function([stypred_cont, stypred_sty, pred_cont_A, pred_sty_A, x_rec, cont, sty],
                                   [stypred_label, y, x, D], original = False)


            total_loss += total_loss_A.item()
            pred = pred_sty_A.data.max(1)[1].cpu()
    ...
    def test(self, datapath,target_data,domain_dict, claim_only=False):

        datafile = os.listdir(datapath)

        # test_loader = normal_dataloader(datapath, target_data, self.batch_size, self.num_worker,
        #                                             self.num_posts)
        test_loader = normal_dataloader_withdomains(datapath, target_data, self.batch_size, self.num_worker, self.num_posts,domain_dict)
        with torch.no_grad():
            start_epoch = self.load(self.model_path, self.checkpoint)
            test_loss_value, Acc_all, Acc1, Prec1, Recll1, F1, Acc2, Prec2, Recll2, F2 = self.evaluation(test_loader, start_epoch, claim_only)
            with open(os.path.join(self.model_path, 'predict.txt'), 'a') as f:
                hist = [Acc_all, Acc1, Prec1, Recll1, F1, Acc2, Prec2, Recll2, F2]
                hist = list(map(str, hist))
                f.write(
    ...
    def all_test(self, datapath, datasets, domain_dict):
        ...
        for dataset in datasets:
            test_loader = normal_dataloader_withdomains(datapath, dataset, self.batch_size, self.num_worker,
                                                        self.num_posts,
                                                        domain_dict)
    ...
    def all_transfer_test(self, dataloader):
        ...
        for iter, sample in enumerate(dataloader):
            ...
            cont, sty, stypred_cont, stypred_sty, pred_cont_A, pred_sty_A, x_rec = self.net(x, False)
            preds = pred_sty_A.data.max(1)[1].cpu()
