...
@Author     : Danke Wu
@File       : DCSC_MD.py
the dataloader is changed to data_withdomain
"""
# -*-coding:utf-8-*-
import sys
import time
...
class Net(object):
    def __init__(self, args, device):
        ...
        # self.net = stymix(self.c_in, self.c_hid, self.num_layers_trans, self.n_head_trans, self.h_hid_trans, self.num_style, self.dropout).to(self.device)
        # self.net = stymix_woemb(self.c_in, self.c_hid, self.num_layers_trans, self.n_head_trans, self.h_hid_trans,
        #                   self.num_style, self.dropout).to(self.device)
        # self.net = stymix_wogrl(self.c_in, self.c_hid, self.num_layers_trans, self.n_head_trans, self.h_hid_trans,self.num_style,
        #                   self.dropout).to(self.device)
        # self.net = stymix_wostyledetection(self.c_in, self.c_hid, self.num_layers_trans, self.n_head_trans, self.h_hid_trans,self.num_style,
        #                   self.dropout).to(self.device)
        self.net = stymix_wodecoder(self.c_in, self.c_hid, self.num_layers_trans, self.n_head_trans, self.h_hid_trans,self.num_style,
                          self.dropout).to(self.device)
        ...
        # self.loss_function = loss_function_wodecoder(self.margin, self.domain_num)
