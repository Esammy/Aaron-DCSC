...
from multi_domains.M3FNED import M3FENDModel
from utils import *
from metrics import *
...
class Net(object):
    def __init__(self, args, device):
        ...
        self.net = M3FENDModel(self.c_in, [self.c_hid], self.dropout, self.filter_num, self.c_hid, self.domain_num, self.device).to(self.device)
    ...
    def train_epoch(self, datapath, start_epoch, domian2idx_dict):
        ...
        for iter, (Nonrumors, Rumors) in enumerate(zip(nonrumor_loader, rumor_loader)):
            xn, yn, dn = Nonrumors
            xr, yr, dr = Rumors
            ...
            label_pred = self.net.save_feature(x,D)
        self.net.init_memory()
    ...
    def save(self, model_path, epoch):
        save_states = {'net': self.net.state_dict(),
                       ...
                       'checkpoint': epoch,
                       'best_mem': self.net.domain_memory.domain_memory
    ...
    def load(self, model_path, checkpoint):
        ...
        self.net.domain_memory.domain_memory = states_dicts['best_mem']
