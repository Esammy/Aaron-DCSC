...
from multi_domains.MMOE import MMoEModel
from utils import *
from metrics import *
...
class Net(object):
    def __init__(self, args, device):
        ...
        # self.net = MultiDomainFENDModel(self.domain_num, self.num_expert,self.c_in, [self.c_hid], self.dropout).to(self.device)
        # self.net = TransE(self.c_in, self.c_hid, self.num_layers_trans, self.n_head_trans, self.h_hid_trans,
        #                   self.dropout).to(self.device)
        self.net = MMoEModel(self.c_in, [self.c_hid], self.dropout, self.filter_num, self.domain_num).to(self.device)
        # self.net = MoSEModel(self.c_in,1, [self.c_hid], self.dropout, self.filter_num, self.domain_num).to(self.device)
        # self.net = TransE(self.c_in, self.c_hid, self.num_layers_trans, self.n_head_trans, self.h_hid_trans,
        #                                           self.dropout).to(self.device)
