...
from multi_domains.MDDA import *
from utils import *
from metrics import *
...
class Net(object):
    def __init__(self, args, device):
        ...
        if self.modelname== 'styledetection':
            self.net = MDDA_stymix_stydetection(self.c_in, self.c_hid, num_layers=1, bidirection=False).to(self.device)
        elif self.modelname == 'styledetection_grl':
            self.net = MDDA_stymix_stydetection_grl(self.c_in, self.c_hid, num_layers=1, bidirection=False).to(self.device)
        elif self.modelname== 'styledecoder':
            self.net = MDDA_stymix_decoder(self.c_in, self.c_hid, num_layers=1, bidirection=False).to(self.device)
        elif self.modelname == 'domaindetection':
            self.net = MDDA_domaindetection(self.c_in, self.c_hid, num_layers=1, bidirection=False, num_domain= self.domain_num).to(self.device)
        ...
        self.mcl = Multilevel_Contrastive_Loss(margin= self.margin,domain_num= self.domain_num)
        # self.mcl = Triple_Loss(margin= self.margin,domain_num= self.domain_num)
        # self.mcl = Quadruple_Loss(margin= self.margin,domain_num= self.domain_num)
    ...
    def train_batch(self, epoch, nonrumor_loader, rumor_loader):
        ...
        for iter, (Nonrumors, Rumors) in enumerate(iterloader):
            ...
            cont, style, stypred_cont, stypred_sty, pred_cont, pred_sty, x_rec = self.net(x,D)
            c_loss = self.celoss(pred_sty, y) + self.celoss(pred_cont,y)
            # x_loss = torch.mean(-torch.log(torch.sigmoid(x_rec) / torch.sigmoid(x)))
            x_loss = F.l1_loss(x_rec,x)
            if  self.modelname== 'styledetection':
                x_mc = self.mcl(stypred_sty, D)
                x_dis = 0
                loss = c_loss + x_loss + x_mc
            elif self.modelname == 'styledecoder':
                x_mc = 0
                x_dis =0
                loss = c_loss + x_loss
                ...
                x_mc = self.mcl(stypred_sty, D)
                x_dis = F.kl_div(torch.log_softmax(stypred_cont, dim = -1), torch.softmax(stypred_sty.detach(), dim=-1))
                loss = c_loss + x_loss + x_mc + x_dis
            elif self.modelname =='domaindetection':
                x_mc = F.cross_entropy(stypred_sty,D)
                x_dis =0
                loss = c_loss + x_loss + x_mc
            ...
            pred = torch.max(pred_sty, dim=1)[1]
            acc = (pred == y).sum()
            acc_all += acc.item()
            n_sample +=len(y)
    ...
    def evaluation(self, dataloader, epoch):
        ...
        for iter, sample in enumerate(dataloader):
            ...
            pred = pred_sty.data.max(1)[1].cpu()
