...
class Net(object):
    def __init__(self, args, device):
        ...
        self.net = DCSC(self.c_in, self.c_hid, self.num_layers_trans, self.n_head_trans, self.h_hid_trans, self.num_style, self.dropout).to(self.device)
        # self.net = DCSC_MLP(self.c_in, self.c_hid, self.num_layers_trans, self.n_head_trans, self.h_hid_trans,
        #                       self.dropout).to(self.device)
        # self.net = DCSC_wogrl(self.c_in, self.c_hid, self.num_layers_trans, self.n_head_trans, self.h_hid_trans,
        #                   self.num_style,self.dropout).to(self.device)
        # self.net = DCSC_wostyledetection(self.c_in, self.c_hid, self.num_layers_trans, self.n_head_trans, self.h_hid_trans,self.num_style,
        #                         self.dropout).to(self.device)
        # self.net = DCSC_claim(self.c_in, self.c_hid, self.num_layers_trans, self.n_head_trans, self.h_hid_trans,
        #                   self.num_style,self.dropout).to(self.device)
        # self.net = DCSC_domaindetection(self.c_in, self.c_hid, self.num_layers_trans, self.n_head_trans, self.h_hid_trans,
        #                   self.domain_num, self.dropout).to(self.device)
        ...
        # self.loss_function = loss_function_wogrl(self.margin, self.domain_num)
        # self.loss_function = loss_function_wostyledetection(self.margin, self.domain_num)

        # self.loss_function = loss_function_tripleLoss(self.margin, self.domain_num)
        # self.loss_function = loss_function_QuadrupleLoss(self.margin, self.domain_num)
        # self.loss_function = loss_function_DomainLoss(self.margin, self.domain_num)
        #
    ...
    def train_epoch(self, datapath, start_epoch, domain_dict):
        ...
        # with torch.no_grad():
        #     test_loader = normal_dataloader_withdomains(datapath, 'val', self.batch_size, self.num_worker, self.num_posts, domain_dict)
        #
    ...
    def train_batch(self, epoch, nonrumor_loader, rumor_loader):
        ...
        for iter, (Nonrumors, Rumors) in enumerate(iterloader):
            ...
            total_loss_A, styloss_cont, styloss_sty, clsloss_cont, clsloss_sty, rec_loss = \
                self.loss_function([stypred_cont, stypred_sty, pred_cont_A, pred_sty_A,  x_rec, cont, sty], [stypred_label, y, x,D])

            # backward
            self.optimizer.zero_grad()
            total_loss_A.backward()
            torch.nn.utils.clip_grad_norm_(self.net.parameters(), max_norm=2)
            self.optimizer.step()

            # # Step B: mixture data
            # cont, sty, x_rec, stypred_cont, stypred_sty, pred_cont_B, pred_sty_B, x_reidx, x_rec= self.net( x_mix.detach(), original=False)
            #
            # stylabels_mask = x_mix.view(B * N, -1)
            # stylabels_mask, = torch.nonzero(stylabels_mask.sum(-1), as_tuple=True)
            #
            # total_loss_B, styloss_cont, styloss_sty, clsloss_cont, clsloss_sty, rec_loss = \
            #     self.loss_function([stypred_cont, stypred_sty, pred_cont_B, pred_sty_B, x_rec,cont, sty], [stypred_label, y, x_reidx, stylabels_mask], original=False)
            # # backward
            # self.optimizer.zero_grad()
            ...
            # acc_sty_B = binary_accuracy(pred_sty_B, y)
            acc_cont_A = binary_accuracy(pred_cont_A, y)
            # acc_cont_B = binary_accuracy(pred_cont_B, y)
